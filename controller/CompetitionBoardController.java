...
public class CompetitionBoardController {
    ...
    public ResponseEntity<?> createBoard(@RequestBody CompetitionBoard board) {
        try {
            ...
            return ResponseEntity.badRequest().body("CompetitionBoard 생성 실패: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> deleteBoardById(@PathVariable Integer id) {
        try {
            ...
            return ResponseEntity.ok("CompetitionBoard가 성공적으로 삭제되었습니다.");
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("CompetitionBoard 삭제 실패: " + e.getMessage());
        }
    }
}
